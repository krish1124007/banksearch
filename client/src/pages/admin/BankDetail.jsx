...
// ─────────────────────────────────────────────────────────────────────────────
// STABLE MODULE-SCOPE CONSTANTS
// These are defined ONCE at module level — never recreated on re-renders.
// Passing new object references to react-chartjs-2 on every render causes
// its internal useEffect to loop, producing "Maximum update depth exceeded".
// ─────────────────────────────────────────────────────────────────────────────

const INTEREST_CHART_OPTIONS = {
  responsive: true,
  animation: false,
  plugins: {
    legend: { position: 'top' },
    title: { display: true, text: 'Interest Rate Comparison' },
  },
};

const TENURE_CHART_OPTIONS = {
  responsive: true,
  animation: false,
  plugins: {
    legend: { position: 'top' },
    title: { display: true, text: 'Loan Tenure Comparison' },
  },
};

// ─────────────────────────────────────────────────────────────────────────────
// SUB-COMPONENTS — defined at MODULE SCOPE (NOT inside BankDetail).
// Defining components inside another component creates a new function reference
// every render → React unmounts/remounts → animations re-trigger → infinite loop.
// ─────────────────────────────────────────────────────────────────────────────

const SectionTitle = ({ icon, title }) => (
  <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2 border-b pb-2 border-gray-200">
    <span className="text-blue-600">{icon}</span> {title}
  </h2>
);

// SectionCard uses pure CSS animation — NO framer-motion whileInView.
// whileInView uses IntersectionObserver; whileHover moves the element which
// re-triggers the observer, causing a setState loop → "Maximum update depth exceeded".
const SectionCard = ({ children, className = "" }) => (
  <section
    className={`bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl shadow-md border border-gray-200 section-card-animate ${className}`}
    style={{
...
const StatCard = ({ title, value, unit = "", color = "blue" }) => (
  <motion.div
    className={`${STAT_COLORS[color]} p-4 rounded-lg`}
...
const BankDetail = () => {
  ...
  // useRef keeps the same object reference across renders — NO new object each render.
  // This prevents react-chartjs-2's internal useEffect from firing on every render.
  const interestRateDataRef = useRef(null);
  const tenureDataRef = useRef(null);
  ...
  // Build chart data only once using useRef — stable reference across all re-renders
  if (!interestRateDataRef.current) {
    interestRateDataRef.current = {
  ...
  if (!tenureDataRef.current) {
    tenureDataRef.current = {
  ...
  const interestRateData = interestRateDataRef.current;
  const tenureData = tenureDataRef.current;

  const renderInterestRates = (rates) => (
    <div className="ml-6 mt-2">
      <Bar
        data={interestRateData}
        options={INTEREST_CHART_OPTIONS}
        height={200}
      />
  ...
  return (
    <>
      {/* CSS keyframe — injected once, no JS animation library needed */}
      <style>{`
        @keyframes fadeInUp {
      ...
      <div className="max-w-7xl mx-auto my-8 p-4 sm:p-8 bg-white rounded-2xl shadow-xl border border-gray-100">
