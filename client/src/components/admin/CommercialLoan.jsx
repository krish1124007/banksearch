...
const CommercialLoan = ({ onChange }) => {
  const [isCommercialEnabled, setIsCommercialEnabled] = useState(false);
  ...
  const resetCommercialLoanState = () => {
    setSelectedOptions({
      under_construction: false,
      builder_purchase_ready: false,
      resale: false,
  ...
  const handleNestedToggle = (option) => {
  ...
  useEffect(() => {
    onChange({
      commercial_loan: isCommercialEnabled,
      ...selectedOptions,
      interest_rate: interest,
      LTV: ltvRanges,
      loan_ticket_size: loanTicketSize
    });
  }, [isCommercialEnabled, selectedOptions, interest, ltvRanges, loanTicketSize]);

  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      {/* Header */}
      <div className={`${isCommercialEnabled ? 'bg-blue-500' : 'bg-gray-100'} px-6 py-4 transition-colors duration-200`}>
        <label className="flex items-center space-x-3 cursor-pointer">
          <div className="relative">
            <input
              type="checkbox"
              checked={isCommercialEnabled}
              onChange={() => {
                setIsCommercialEnabled((prev) => {
                  const next = !prev;
                  if (!next) resetCommercialLoanState();
                  return next;
                });
              }}
              className="sr-only"
          ...
          <span className={`text-xl font-semibold ${isCommercialEnabled ? 'text-white' : 'text-gray-700'}`}>
            Commercial Loan
          </span>
        </label>
      </div>
      ...
      {isCommercialEnabled && (
        <div className="p-6 bg-gray-50 space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                ["Builder Purchase Ready", "builder_purchase_ready"],
                ["Resale", "resale"]
              ].map(([label, key]) => (
                  ...
                  <div className={`flex items-center space-x-3 p-3 rounded-lg border-2 transition-all duration-200 ${
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={() => handleNestedToggle(key)}
                        className="sr-only"
                      />
                    ...
                    <span className={`text-sm font-medium ${
                      selectedOptions[key] ? 'text-blue-700' : 'text-gray-700'
                    }`}>
                      {label}
                    </span>
                  </div>
          ...
          {/* LTV (kept as regular text input as requested) */}
          <div>
